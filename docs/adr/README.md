# Architecture Decision Records

Index of architectural decisions for adrai (AIDE Debate Tracking System).

---

## ADR Index

| ADR | Title | Status | Date |
|-----|-------|--------|------|
| [ADR-001](ADR-001-technology-stack.md) | Technology Stack | Accepted | 2026-01-29 |
| [ADR-002](ADR-002-architecture-style.md) | Architecture Style | Accepted | 2026-01-29 |

---

## Status Legend

| Status | Meaning |
|--------|---------|
| Proposed | Under discussion |
| Accepted | Decision made, in effect |
| Deprecated | No longer applies |
| Superseded | Replaced by another ADR |

---

## Creating New ADRs

1. Copy template from `./templates/adr-template.md`
2. Use next available number: `ADR-003`
3. Name file: `ADR-003-brief-title.md`
4. Add entry to this index
5. Link to related debates (`Resolved by: DEB-NNNN`) if applicable

---

## Related

- [adrai Conceptual Overview](../adrai-conceptual-overview.md)
- [Constraints](../constraints.md)
- [Architecture Overview](../software-architecture/overview.md)
